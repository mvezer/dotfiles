...
function M.setup()
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function(args)
      vim.keymap.set("n", "<leader>lkc", function()
        M.copy()
      end, { buffer = args.buf, desc = "Kotlin: copy fully qualified class name" })

      vim.keymap.set("n", "<leader>lkC", function()
        M.copy({ binary = true })
      end, { buffer = args.buf, desc = "Kotlin: copy JVM binary name (Outer$Inner)" })
    end,
  })
end
